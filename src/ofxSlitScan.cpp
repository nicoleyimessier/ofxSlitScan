...
void ofxSlitScan::drawVerticalSlotIn(int num_cols, glm::vec2 size, ofTexture& tex_ref, ofMesh& mesh,
    ...
    for (int i = 0; i < num_cols; i++) {
        ...
        float offSsetExp_map = ofMap(offSsetExp, 0, 1, 11.0f, (1 - size_percent));
...
void ofxSlitScan::drawHorizontalPixel(glm::vec2 pos, glm::vec2 size, ofTexture& tex_ref, ofMesh& mesh, float proto_val0, float proto_val1) {
    MeshHelper::RectData where;
    where.size.x = size.x;
    where.size.y = size.y;
    where.pos.x = pos.x; 
    where.pos.y = pos.y + proto_val0 * proto_val1;
    ...
    temp.verts.bl.y = where.pos.y + where.size.y * ofMap(proto_val0, 0, 1, 1, 0);

    temp.verts.br.x = where.pos.x + where.size.x;
    temp.verts.br.y = where.pos.y + where.size.y * ofMap(proto_val0, 0, 1, 1, 0);

    //! TEXTURE
    temp.texCoords.tl.x = tex.pos.x;
    temp.texCoords.tl.y = tex.size.y * proto_val0;

    temp.texCoords.tr.x = tex.pos.x + tex.size.x;
    temp.texCoords.tr.y = tex.size.y * proto_val0;
    ...
    pixel_stretch.verts.tl.y = where.pos.y + where.size.y * ofMap(proto_val0, 0, 1, 1, 0);

    pixel_stretch.verts.tr.x = where.pos.x + where.size.x;
    pixel_stretch.verts.tr.y = where.pos.y + where.size.y * ofMap(proto_val0, 0, 1, 1, 0);
